...
public class test4 {
    ...
    public void testFindById() {
        // 通过id查询
        User user = userDao1.findById(50);
        System.out.println(user);
    }
    ...
    public void testFindByName() {
        // 通过id查询
        List<User> users = userDao1.findByName("%王%");
    ...
    public void testFindByQueryVo() {
        ...
        user.setUsername("%王%");
        vo.setUser(user);
        List<User> users = userDao1.findUserByVo(vo);
        for (User u : users) {
    ...
    public void testFindByCondition() {
        User user = new User();
        user.setUsername("老王");
        List<User> users = userDao1.findUserByCondition(user);
        for (User u : users) {
            System.out.println(u);
        }
    }
    ...
    public void testFindInIds() {
        ...
        list.add(44);
        vo.setIds(list);
        List<User> users = userDao1.findUserInIds(vo);
        for (User u : users) {
            System.out.println(u);
        }
    }
